# Help and Documentation for ETERNA E-Archive

Welcome to ETERNA's help documentation! Here you'll find all the information you need to use, administer, and develop the ETERNA E-Archive system. The documentation is organized into different categories to make it easy to find the right information.

## Table of Contents

### Reference Documentation
Technical references and detailed information about the system.

- **[Overview](reference/Overview.md)** - An introduction to ETERNA and its core functions
- **[Descriptive Metadata Types](reference/Descriptive_Metadata_Types.md)** - Different types of metadata supported
- **[Metadata Formats](reference/Metadata_Formats.md)** - Formats supported for metadata
- **[Representation Information](reference/Representation_Information.md)** - Information about digital representation
- **[Statistics](reference/Statistics.md)** - Statistics and reporting in the system
- **[User Configuration](reference/User_Configuration.md)** - Configuration options for users
- **[License](reference/LICENSE.md)** - License information for the project

### User Guides
Step-by-step guides for using ETERNA effectively.

- **[Quickstart](usage/Quickstart.md)** - Get started quickly with ETERNA
- **[Edit Descriptive Metadata](usage/EditDescriptiveMetadata.md)** - How to edit metadata
- **[Advanced Search](usage/Advanced_Search.md)** - Advanced search features
- **[Pre-Ingest](usage/Pre_Ingest.md)** - Preparing data for archiving
- **[PDF Redaction](usage/PdfRedaction.md)** - How to redact sensitive content in PDF files
- **[Troubleshooting](usage/Troubleshooting.md)** - Solutions to common problems
- **[FAQ](usage/FAQ.md)** - Answers to frequently asked questions
- **[README](usage/README.md)** - General information about ETERNA

### Administration Guides
Guides for system administrators and archive managers.

- **[Portal Administration](administration/Portal_Administration.md)** - Administer the ETERNA portal
- **[Central Authentication Service](administration/Central_Authentication_Service.md)** - Manage user authentication
- **[Disposal Policies](administration/Disposal_Policies.md)** - Policies for data management
- **[Disposal](administration/Disposal.md)** - Processes for data management
- **[Permissions](administration/Permissions.md)** - Managing user permissions
- **[Risk Assessment](administration/Risk_Assessment.md)** - Evaluation of archiving risks

### Developer Resources
Documentation for developers who want to contribute to or extend ETERNA.

- **[Developer's Guide](development/Developers_Guide.md)** - Guide for developers