  ...
  public void resolve(List<String> historyTokens, AsyncCallback<Widget> callback) {
    if (historyTokens.size() > 2) {
      ...
      CompletableFuture<IndexedRepresentation> retrieveRepresentationFuture = services.rodaEntityRestService(
        s -> s.findByUuid(representationId, LocaleInfo.getCurrentLocale().getLocaleName()),
      ...
      CompletableFuture.allOf(retrieveFileFuture, retrieveAIPFuture, retrieveRepresentationFuture)
        .whenComplete((unused, throwable) -> {
          if (throwable != null) {
            ...
            pdfRedactor.setupNavigation(retrieveFileFuture.join(), retrieveAIPFuture.join(),
              retrieveRepresentationFuture.join());
