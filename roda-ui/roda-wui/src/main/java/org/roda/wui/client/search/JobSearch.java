...
public class JobSearch extends SimplePanel {
  ...
  public JobSearch(String jobsListId, String jobReportsListId, Filter defaultJobFilter, Filter defaultJobReportFilter,
    boolean isIngest, HistoryResolver jobResolver, List<Actionable.Action<IndexedJob>> actionWhitelist,
    List<Actionable.Action<IndexedJob>> actionBlacklist) {
    JobActions jobActions = null;
    if (jobResolver != null) {
      jobActions = JobActions.get(jobResolver);
    }
    ...
    ListBuilder<IndexedJob> jobListBuilder = new ListBuilder<>(() -> new JobList(),
      new AsyncTableCellOptions<>(IndexedJob.class, jobsListId).withFilter(new Filter(defaultJobFilter))
        .withAutoUpdate(5000).withActionable(jobActions).withActionWhitelist(actionWhitelist)
        .withActionBlacklist(actionBlacklist).bindOpener());
