...
This is a full-service ETERNA deployment example including a Traefik reverse proxy with TLS termination, ZooKeeper, Solr in cloud mode, ClamAV, and Siegfried. Optional services (CAS SSO, Swagger UI, SFTP drop folder) are included as commented-out blocks in the compose file.
...
Data in `./data/` and `./integration/` is preserved on the host. Named volumes (Solr, ZooKeeper, ClamAV, Siegfried) are preserved in Docker volume storage. To remove volumes as well:
