 ...
 */
public class PekkoBackgroundJobActor extends PekkoBaseActor {

  private static final Logger LOGGER = LoggerFactory.getLogger(PekkoBackgroundJobActor.class);

  private ActorRef jobsManager;

  private SupervisorStrategy strategy = new OneForOneStrategy(false, DeciderBuilder.matchAny(e -> {
    LOGGER.error("A child actor of {} has thrown an exception", PekkoBackgroundJobActor.class.getSimpleName(), e);
  ...
  /** Public constructor */
  public PekkoBackgroundJobActor(ActorRef jobsManager) {
    super();
