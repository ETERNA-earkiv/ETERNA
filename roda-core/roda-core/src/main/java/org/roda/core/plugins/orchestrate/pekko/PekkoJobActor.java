...
public class PekkoJobActor extends PekkoBaseActor {
  private static final Logger LOGGER = LoggerFactory.getLogger(PekkoJobActor.class);

  private ActorRef jobsManager;

  private SupervisorStrategy strategy = new OneForOneStrategy(false, DeciderBuilder.matchAny(e -> {
    LOGGER.error("A child actor of {} has thrown an exception", PekkoJobActor.class.getSimpleName(), e);
  ...
  /** Public constructor */
  public PekkoJobActor(ActorRef jobsManager) {
    super();
