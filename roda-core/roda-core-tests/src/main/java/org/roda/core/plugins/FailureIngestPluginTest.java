...
import org.roda.core.plugins.base.v2.FailureOnMandatoryStepAfterPartialSuccessIngestPlugin;
...
public class FailureIngestPluginTest {
  ...
  public void setUp() throws Exception {
    ...
    URL corporaURL = FailureIngestPluginTest.class.getResource("/corpora");
    corporaPath = Paths.get(corporaURL.toURI());

    LOGGER.info("Running FailureIngestPlugin tests under storage {}", basePath);
  }
  ...
  private void assessAIP(AIP aip)
    ...
    Assert.assertEquals(preservationEventsFind.getTotalCount(), 6L);
    ...
    Assert.assertEquals(expectedEvents.size(), 2);
